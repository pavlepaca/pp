        ...
        while(running){
            ...
            for(size_t i=0; i<kolizije.size();i++){
                   bool p = sp->provera(kolizije[i]);
                    if(p == true){
                        if(kolizije[i]->isKey()){
                            drawables.erase(drawables.begin()+5);
                            kolizije.erase(kolizije.begin()+3);
                            SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION,"Obavestenje","Imate kljuc",window);
                            cout<<"Pokupili ste kljuc"<<endl;
                            imaKljuc=true;
                        }
                        else if(kolizije[i]->isChest() && imaKljuc==true){
                            SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION,"Kraj igrice","Pobedili ste",window);
                            running = false;
                        }
                        else if(kolizije[i]->isEnemy()) {
                        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION,"Izgubili ste","Ubio vas je kostur",window);
                            running = false;
                            }
            ...
            for(size_t i=0;i<blato.size();i++){
                ...
                if (p){
                    ...
                    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION,"Kraj igrice","Izgubili ste",window);
                    running = false;
                }
                else if(!p){
                    continue;
